...
def main():
    ...
    while(1):
        ...
        tmp_as = -1
        while (1):
            tmp_as = context.find(answers,tmp_as+1)
            if tmp_as == -1 : break
            answer_starts.append(tmp_as)
        if len(answer_starts) > 1 :
            for index, tmp_as in  enumerate(answer_starts):
                print(index, context[tmp_as:tmp_as+10])
            answer_starts_index = int(input('selecte answer: '))
            answer_start = answer_starts[answer_starts_index]
        else:
